...
public class MeuTST<Value extends Comparable<Value>> {
    ...
    public Iterable<String> keysWithPrefixByValue(String prefix) {
        ...
        while(!pq.isEmpty())
            queue.enqueue(pq.delMax().prefix);
        return queue;
    }

    private void collect(Node<Value> x, StringBuilder prefix, MaxPQ<Prefix> pq) {
        if (x == null) return;
        collect(x.left,  prefix, pq);
        String s = prefix.toString() + x.c;
        if (x.val != null) pq.insert(new Prefix() {{
            this.prefix = s;
            this.val = x.val;
        }});
    ...
    private Node<Value> delete(Node<Value> x, String key, int d) {
        ...
        if (c < x.c)
            ...
            if (d < key.length() - 1) x.mid = delete(x.mid, key, d+1);
            else if(x.val != null) {
                x.val = null;
                n--;
            }
